...
function computeStreak(logs: LogEntry[]): number {
  ...
  const days = new Set(logs.map((log) => new Date(log.created_at).toDateString()));
  ...
  return (
    <section className="dashboard-page">
      ...
      <div className="dashboard-page__stats-grid">
        ...
        />
        <StatCard label="Streak" value={String(streak)} trend={streak > 0 ? "days in a row" : "log today to start"} />
      </div>
